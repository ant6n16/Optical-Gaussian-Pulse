%%%% CODIGO TREN PULSOS - FRECUENCIA INTERMEDIA %%%%
...
% Estructura de datos:
N_freq = 2.^16; 
f_i = linspace(f_0-0.03e12, f_0+3.97e12, N_freq+1).'; 
...
% quepan, tendria que poner mas puntos en la estructura de dato
...
for ii=1:k
        ...
        pulsos(:,ii) = secuencia(ii).*A_0.*exp(-(t_i-tau_d(ii)).^2.*inv(2*tau_0.^2)).*cos(2.*pi.*(f_0-f_i(1,1)).*(t_i-tau_d(ii)));
